...
int main(int argc, char *argv[]) {
	...
	while ((c = getopt(argc, argv, "vp:n:u")) != -1) {
		switch(c) {
                case 'v':
                        verbose = 1;
                        break;
	...
	if (pid) {
		pprintf("just before mprotect\n");
		pause();
		pprintf("mprotect %d\n", getpid());
		if (mprotect(p, HPS - (unaligned * HPS/2), PROT_READ|PROT_WRITE|PROT_EXEC))
			err("mprotect");
		pprintf("mprotect done\n");
	}

	/*
	if (!pid) {
		pprintf("mprotect\n");
		if (mprotect(p, HPS, PROT_READ|PROT_WRITE|PROT_EXEC))
			err("mprotect");
		pprintf("mprotect done\n");
		pause();
		return 0;
